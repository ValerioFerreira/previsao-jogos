...
function Column({ team, players, teamIds, showFin }: { team: string; players: ScorerPlayer[]; teamIds: Record<string, number>; showFin: boolean }) {
  ...
  return (
    <div className="bg-card border border-border/50 rounded-xl p-4">
      ...
      {/* Cabeçalho de grupos: Marcar | Finalizar (≥) — separação visual */}
      {showFin && (
        <div className="flex items-end gap-2 text-[9px] uppercase tracking-wide text-muted-foreground/80">
          ...
          <span className="shrink-0 border-l border-border/40 pl-2 text-center text-cyan-400/80 font-semibold" style={{ width: "8.25rem" }}>Finalizar (≥)</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-muted-foreground pb-1 border-b border-border/30">
        ...
        <span className="shrink-0 w-12 text-right">{showFin ? "Prob." : "Prob."}</span>
        {showFin ? (
...
export default function ScorersCard({ data, home, away, teamIds, embedded = false }: {
  ...
  const grid = (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Column team={home} players={hp || []} teamIds={teamIds} showFin={showFin} />
      <Column team={away} players={ap || []} teamIds={teamIds} showFin={showFin} />
    </div>
  );
  ...
  return (
    <div className="mt-8">
      <h4 className="text-sm font-bold uppercase text-foreground mb-4 flex items-center justify-center gap-1.5">
        Jogador
        <InfoTooltip text="Por jogador do elenco recente, se jogar: P(marcar a qualquer momento) e P(finalizações ≥ 0,5/1,5/2,5) — modelos de goleador e de finalizações (forma + defesa do adversário + mando + minutos), calibrados. Odd justa = 1/probabilidade, sem margem de casa." />
      </h4>
      {grid}
    </div>
  );
