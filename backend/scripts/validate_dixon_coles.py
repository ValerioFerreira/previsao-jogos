...
# -*- coding: utf-8 -*-
"""
scripts/validate_dixon_coles.py
===============================
Compara o modelo Dixon-Coles Binomial Negativo (DC-NB) contra o modelo
quantílico/classificadores atuais na validação temporal justa.
Gera relatórios, métricas de calibração (ECE, Brier, Log-Loss) e gráficos.
"""
...
COMP_DATA_PATH = ARTIFACTS_DIR / "dixon_coles_comparison.json"
...
def compute_quantile_goals_distribution(pred_q10, pred_q50, pred_q90, max_goals=12):
    """
    Deriva a distribuição discreta de gols totais de cada jogo a partir dos quantis do modelo atual
    e retorna uma matriz de probabilidades de shape (N, max_goals + 1).
    """
    N = len(pred_q50)
    ...
    for i in range(N):
        ...
        # Discretizar a distribuição normal integrando nos bins [k-0.5, k+0.5]
        p_k = norm.cdf(k + 0.5, loc=mu, scale=std) - norm.cdf(k - 0.5, loc=mu, scale=std)
        p_k[p_k < 0] = 0.0
        if p_k.sum() > 0:
            p_k /= p_k.sum()  # Renormalização explícita sobre a matriz finita
        else:
            p_k[0] = 1.0  # fallback
...
def main():
    print("================================================================================")
    print("VALIDAÇÃO COMPLEMENTAR: MODELO DIXON-COLES BINOMIAL NEGATIVO (GOLS)")
    print("================================================================================")
    ...
    base_feats, _ = split_feature_sets = (all_feats, None) # Apenas base_feats é usada para gols
    base_feats = [c for c in all_feats if "sb_" not in c]
    
    # Divisão temporal (mesma data de corte do diagnóstico da Fase 1)
    ...
    # Salvar o modelo em paralelo
    dc_model.save(ARTIFACTS_DIR / "dixon_coles_goals.joblib")
    ...
    prob_res_dc = dc_probs["result"]  # shape (N, 3) correspondente a [A, D, H]
    prob_btts_dc = dc_probs["btts"]
    prob_ov_dc = dc_probs["over_2_5"]
    
    # Calcular distribuição de probabilidade de gols totais sob Dixon-Coles
    # Somando as diagonais secundárias da matriz conjunta
    P_joint_dc = dc_probs["joint"] # shape (N, M+1, M+1)
    ...
    for i in range(len(df_test)):
        ...
        # Renormalização final por segurança
    ...
    # 4. Gols Totais (Log-Loss sobre a distribuição discreta)
    ...
    # MAE/RMSE (Diagnósticos adicionais de acerto pontual)
    # Modelo Atual prediz a mediana (q50)
    mae_curr = mean_absolute_error(y_test_goals, pred_q50)
    rmse_curr = np.sqrt(mean_squared_error(y_test_goals, pred_q50))
    # Dixon-Coles prediz a média conjunta estimada
    ...
    print("\n" + "="*80)
    print(" TABELA COMPARATIVA DE DESEMPENHO PROBABILÍSTICO (TESTE TEMPORAL OUT-OF-SAMPLE)")
    print("="*80)
    ...
    # --------------------------------------------------------------------------
    # 6. Gerar Gráficos de Calibração Comparativos
    # --------------------------------------------------------------------------
    print("\n>> Salvando curvas de calibração comparativas...")
    ...
    plt.tight_layout()
    plot_path = PLOTS_DIR / "compare_calibration_dc.png"
