...
from app.domains.auth.deps import client_ip, get_db, require_admin, require_owner
...
def user_detail(user_id: str, _: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def unblock(user_id: str, request: Request, admin: User = Depends(require_owner), db: Session = Depends(get_db)):
...
@router.post("/users/{user_id}/role", response_model=schemas.OkResponse)
def update_user_role(user_id: str, data: schemas.UserRoleUpdateRequest, request: Request,
                     admin: User = Depends(require_owner), db: Session = Depends(get_db)):
    service.update_user_role(db, admin, user_id, data.role, client_ip(request))
    return schemas.OkResponse(detail="Papel do usuário atualizado com sucesso.")
...
def transactions(user_id: str | None = None, limit: int = Query(50, ge=1, le=200),
                 offset: int = Query(0, ge=0), _: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def analytics_dashboard(_: User = Depends(require_owner), db: Session = Depends(get_db)):
...
# ---------- cupons ----------
@router.get("/coupons")
def list_coupons(_: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def coupon_analytics(_: User = Depends(require_owner), db: Session = Depends(get_db)):
    return service.coupon_analytics(db)


# ---------- parceiros (afiliados) ----------
@router.get("/affiliates")
def list_affiliates(status: str | None = None, _: User = Depends(require_owner), db: Session = Depends(get_db)):
    return service.list_affiliates(db, status)


@router.get("/affiliates/{affiliate_id}")
def affiliate_detail(affiliate_id: str, _: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def demo_usage(_: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def pending_counts(_: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def list_affiliate_payments(affiliate_id: str, _: User = Depends(require_owner), db: Session = Depends(get_db)):
    return service.list_affiliate_payments(db, affiliate_id)


# ---------- pacotes de crédito ----------
@router.get("/packages")
def list_packages(_: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def list_campaigns(_: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def campaign_dashboard(campaign_id: str, _: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def list_tickets(status: str | None = None, _: User = Depends(require_owner), db: Session = Depends(get_db)):
...
@router.get("/banners")
def list_banners(_: User = Depends(require_owner), db: Session = Depends(get_db)):
